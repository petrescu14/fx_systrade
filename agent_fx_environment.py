...
import sklearn
import talib as ta
from numpy.testing._private.utils import numpy
...
class FXEnvironment:
    def __init__(self):
        ...
        # チャートのタイプ判定に使う過去データの長さ
        self.CHART_TYPE_JDG_LEN = 25

        self.VALIDATION_DATA_RATIO = (
            1.0  # rates of validation data to (all data - train data)
        )

        # データの先頭から何番目のデータを取得するか
        ...
        self.FEATURE_NAMES = [
            # 特徴量のリスト
            "diff_ratio_between_previous_rate",
            "rsi",
            "ma",
            "ma_kairi",
            "bb_1",
        ...
        # 日時のリスト
        self.exchange_dates: np.array = None
        # 為替のリスト
        self.exchange_rates: np.array = None
    ...
    def preprocess_data(self, X):
        ...
        Returns:
            X_T(np.array): 標準化されたリスト
            scaler(StandardScaler): 標準化されたデータ
    ...
    def judge_chart_type(self, data_arr):
        ...
        if (
            max_idx != 0
            and max_idx != last_idx
            and min_idx != 0
            and min_idx != last_idx
        ):
            return 0
    ...
    def get_rsi(self, price_arr, cur_pos, period=40):
        ...
        Args:
            price_arr (np.array): 為替のリスト
            cur_pos (int): 現在の位置
            period (int, optional): RSIの期間. Defaults to 40.
        ...
        return ta.RSI(prices, timeperiod=period)[-1]  # pyright: ignore
    ...
    def get_ma(self, price_arr, cur_pos, period=20):
        ...
        Args:
            price_arr (np.array): 為替のリスト
            cur_pos (int): 現在の位置
            period (int, optional): 移動平均の期間. Defaults to 20.
        ...
        return ta.SMA(prices, timeperiod=period)[-1]  # pyright: ignore
    ...
    def get_bb_1(self, price_arr, cur_pos, period=40):
        ...
        Args:
            price_arr (np.array): 為替のリスト
            cur_pos (int): 現在の位置
            period (int, optional): ボリンジャーバンドの期間. Defaults to 40.
        ...
        return ta.BBANDS(prices, timeperiod=period)[0][-1]  # pyright: ignore
    ...
    def get_bb_2(self, price_arr, cur_pos, period=40):
        ...
        return ta.BBANDS(prices, timeperiod=period)[2][-1]  # pyright: ignore
    ...
    def get_mo(self, price_arr, cur_pos, period=20):
        ...
        Args:
            price_arr (np.array): 為替のリスト
            cur_pos (int): 現在の位置
            period (int, optional): MOの期間. Defaults to 20.
        ...
        return ta.CMO(prices, timeperiod=period)[-1]  # pyright: ignore
    ...
    # Todo: historical_volatility という名前にする
    def get_vorarity(self, price_arr, cur_pos):
        """ボラティリティを取得する関数
    ...
    def is_weekend(self, date_str):
        ...
        Args:
            date_str (string): Time (UTC)
            ex) 2003.08.03 21:00:00

        Returns:
            bool: 土日ならTrue, それ以外ならFalse
    ...
    def make_serialized_data(self, start_idx, end_idx, step, x_arr_fpath, y_arr_fpath):
        """シリアライズ化されたデータを作成する関数

        Args:
            start_idx (int): 開始インデックス
            end_idx (int): 終了インデックス
            step (int): ステップ
            x_arr_fpath (string): x_arrのファイルパス
            y_arr_fpath (string): y_arrのファイルパス
        ...
        for i in range(start_idx, end_idx, step):
            ...
            input_mat.append(
                [
                    ...
                    (self.exchange_rates[i] - self.exchange_rates[i - 1])
            ...
            if y_arr_fpath != None:
                tmp = (
                    self.exchange_rates[i + self.PREDICT_FUTURE_LEGS]
                    - self.exchange_rates[i]
                )
    ...
    def setup_serialized_fx_data(self):
        ...
        if os.path.exists("./exchange_rates.pickle"):
            ...
            for line in rates_fd:
                ...
                if (
                    splited[2] != "High"
                    and splited[0] != "<DTYYYYMMDD>"
                    and splited[0] != "204/04/26"
                    and splited[0] != "20004/04/26"
                    and self.is_weekend(splited[0]) == False
                ):
        ...
        if os.path.exists("./all_input_mat.pickle"):
            ...
            all_input_mat, all_angle_mat = self.make_serialized_data(
                self.DATA_HEAD_ASOBI,
                len(self.exchange_rates)
                - self.DATA_HEAD_ASOBI
                - self.PREDICT_FUTURE_LEGS,
                self.SLIDE_IDX_NUM_AT_GEN_INPUTS_AND_COLLECT_LABELS,
                "./all_input_mat.pickle",
                "./all_angle_mat.pickle",
            )
        ...
        print(
            "data size of all rates for train and test: "
            + str(len(self.exchange_rates))
        )
    ...
    def get_env(self, type_str, reward_gamma=0.95):
        if type_str == "backtest":
            return self.InnerFXEnvironment(
                self.tr_input_arr,
                self.exchange_dates,
                self.exchange_rates,
                self.DATA_HEAD_ASOBI,
                idx_step=1,  # idx_step=self.PREDICT_FUTURE_LEGS,
                angle_arr=self.tr_angle_arr,
    ...
    class InnerFXEnvironment:
        def __init__(
            ...
            angle_arr=None,
            half_spred=0.0015,
            holdable_positions=100,
            performance_eval_len=20,
            reward_gamma=0.95,
            is_backtest=False,
        ):
            self.NOT_HAVE = 0
            ...
            self.log_fd_bt = open(
                "./backtest_log_" + dt.now().strftime("%Y-%m-%d_%H-%M-%S") + ".txt",
                mode="w",
            )
            ...
            self.portfolio_mngr = PortforioManager(
                exchange_rates, self.half_spread, holdable_positions
            )
            ...
            self.base3_max_float = float(
                int("".join(["2"] * (self.performance_eval_len - 1)), 3)
            )
        ...
        def get_recent_rewards_sum(self, episode_idx):
            if self.cur_idx < self.performance_eval_len:
                ...
                calc_list = self.won_pips_to_calculate_sratio[
                    episode_idx - self.performance_eval_len + 1 : episode_idx + 1
                ]
                return sum(calc_list)
                # return sum(calc_list) / (np.std(np.array(calc_list)) + 0.00001)

        def close_all(self, cur_episode_rate_idx):
            won_pips, won_money, each_pos_won = self.portfolio_mngr.close_all(
                cur_episode_rate_idx
            )

            for idx in range(0, len(self.positions_identifiers)):
                # won_pipsを記録しておく（過去のepisodeも含めた獲得pips総計等を計算する前に）
                self.won_pips_to_calculate_sratio[
                    each_pos_won[idx][1] - self.idx_geta
                ] = each_pos_won[idx][0]
                ...
                self.additional_infos.append(
                    [
                        self.positions_identifiers[idx],
                        each_pos_won[idx][0],
                        episode_idx_of_past_open,
                        each_pos_won[idx][2],
                    ]
                )
            ...
            for idx in range(0, len(self.donot_identifiers)):
                self.additional_infos.append(
                    [self.donot_identifiers[idx], 0, self.donot_episode_idxes[idx]],
                    self.NOT_HAVE,
                )
        ...
        def step(self, action_num):
            ...
            if action == "BUY":
                ...
                if self.portfolio_mngr.additional_pos_openable():
                    ...
                    a_log_str_line += (
                        ",OPEN_LONG"
                        + ",0,0,"
                        + str(self.exchange_rates[cur_episode_rate_idx])
                        + ","
                        + str(buy_val)
                    )
                    ...
                    a_log_str_line += (
                        ",POSITION_HOLD,0,"
                        + str(
                            self.portfolio_mngr.get_evaluated_val_diff_of_all_pos(
                                cur_episode_rate_idx
                            )
                        )
                        + ","
                ...
                if len(self.positions_identifiers) > 0:
                    if self.portfolio_mngr.having_long_or_short == self.LONG:
                        ...
                        self.portfolio_mngr.having_long_or_short == self.NOT_HAVE
                    ):  # DONOTのダミーポジションだけ存在する場合
            ...
            a_log_str_line += (
                ","
                + str(self.portfolio_mngr.get_current_portfolio(cur_episode_rate_idx))
                + ","
                + str(self.portfolio_mngr.total_won_pips)
                + ","
                + str(self.portfolio_mngr.having_money)
                + ","
                + str(self.portfolio_mngr.get_nomal_position_num())
            )
            ...
            if (self.idx_geta + self.cur_idx) >= len(self.input_arr):
                ...
                self.logfile_writeln_bt(
                    "excecution time of backtest: " + str(process_time)
                )
                self.logfile_writeln_bt(
                    "result of portfolio: "
                    + str(
                        self.portfolio_mngr.get_current_portfolio(cur_episode_rate_idx)
                    )
                )
                print(
                    "result of portfolio: "
                ...
                return (
                    ...
                    reward,
                    True,
                    [cur_step_identifier] + self.additional_infos,
                    False,
                )
            else:
                ...
                if (
                    len(self.positions_identifiers) >= self.holdable_positions * 2
...
class PortforioManager:
    ...
    def buy(self, rate_idx):
        ...
        currency_num = (
            self.having_money / (self.holdable_position_num - self.position_num)
        ) / trade_val
    ...
    def get_evaluated_val_diff_of_all_pos(self, rate_idx):
        ...
        for position in self.positions:
            if position[1] == self.LONG:
                total_evaluated_money_diff += position[2] * (
                    (cur_price_no_spread - self.half_spread) - position[0]
                )
            else:  # self.SHORT
                total_evaluated_money_diff += position[2] * (
                    position[0] - (cur_price_no_spread + self.half_spread)
                )
    ...
    def get_current_portfolio(self, rate_idx):
        ...
        for position in self.positions:
            if position[1] == self.LONG:
                # 売った際に得られる現金で評価
                total_evaluated_money += position[2] * (
                    cur_price_no_spread - self.half_spread
                )
